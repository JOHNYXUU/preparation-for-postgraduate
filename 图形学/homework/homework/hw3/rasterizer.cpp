...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
            ...
            for(int j=lowbound;j<=upbound;j++)
            {
                ...
                if(cnt)
                {
                    ...
                    if(z_interpolated<depth_buf[get_index(i,j)])
                    {
                        ...
                        fragment_shader_payload fsp(interpolated_color,interpolated_normal.normalized(),interpolated_texcoords,t.tex);
                        fsp.view_pos = interpolated_shadingcoords;
                        auto pixelcolor = fragment_shader(fsp);
                        frame_buf[get_index(i,j)] = pixelcolor;
                    }
                }
            }
    }
    else
    {
        for(int i=leftbound;i<=rightbound+1;i++)
            for(int j=lowbound;j<=upbound+1;j++)
            {
                float x = i+0.5;
                float y = j+0.5;
                // std::cout << x << ' '<<y<<' '<<insideTriangle(x,y,V)<<'\n';
                if(insideTriangle(x,y,t.v))
                {
                    ...
                    {   
                        ...
                        fragment_shader_payload fsp(interpolated_color,interpolated_normal.normalized(),interpolated_texcoords,texture?&*texture:nullptr);
                        fsp.view_pos = interpolated_shadingcoords;
                        auto pixelcolor = fragment_shader(fsp);
                        frame_buf[get_index(i,j)] = pixelcolor;
                        Vector2i p(i,j);
                        set_pixel(p,pixelcolor);
                        // frame_buffer[get_index(i,j)] = color;
